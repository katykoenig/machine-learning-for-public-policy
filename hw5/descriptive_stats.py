...
def summary_stats(dataframe):
    ...
    Output: a table
    '''
    fn.check_for_funding(dataframe, timeframe=60)
    summary = dataframe.describe()
    dataframe.drop(columns='funded_by_deadline', inplace=True)
    return summary
...
def create_scatterplots(dataframe, unique_id='projectid'):
    '''
...
def combine_des_stats(dataframe):
    '''
    Function to combine the above description statistics functions above and
    print their outputs

    Inputs: a pandas dataframe

    Outputs: Prints summaries of the basic statistics, null values and
             correlations in the dataframe
    '''
    sum_stats = summary_stats(dataframe)
    print("Summary Statistics")
    print(sum_stats)
    null_summ = check_null_values(dataframe)
    print("Summary of Null Values")
    print(null_summ)
    corr_df = evaluate_correlations(dataframe, 'correlations.png')
    print("Correlations")
    print(corr_df)
